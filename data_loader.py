...
class BertDataset(Dataset):
    """
    This class will create dataset for training Bert based Model,and it's inherited 
    from map-style dataset. Each instance that include input_id,masking_input,
    segmentation for job,company_profile,also has title text,meta data,label

    Args:
     examples(iterable object):Each instance in examples must be a namedTuple(InputFeature).
     tokenizer(Prtrained Tokenizer object):This object is come from transformer that corresponding model.
     lda_vocab_path(str):The path for Loading gensim Dicitonary class.
     lda_model_path(str):The path for Loading gensim LDA class.
     args:Build from ArgumentParser class.

    Attributes:
     data(list of InputFeature):The object contain all InputFeature object.
     tokenizer(PretrainTokenizer object):The tokenizer that corresponding pretrain model.
     item_vocab(list of str):This is job title field's vocab.
     lda_vocab(gensim Dicitonary instance):The lda_vocab that build from topic_model module.
     lda_model(gensim LDA instance):The lda_model that build from topic_model module.
    ...
    def __getitem__(self, key):
        """
        It's will return NamedTuple instance based on Bert Input format.
        Also you can use indexing method to query nth's data.
        """
        # fetch data
        ...
        # combined data
        job_tokens = [self.tokenizer.cls_token_id] + \
            allToken_ids[1]+allToken_ids[2]
        cp_tokens = [self.tokenizer.cls_token_id] + \
            allToken_ids[0]+allToken_ids[3]
        ...
        if desc_bow:
            ...
            desc_topics = corpus2dense(
                [desc_topics], num_terms=self.lda_model.num_topics, num_docs=1).T.tolist()[0]
        else:
            desc_topics = [0.0]*self.lda_model.num_topics

        # convert title to index
        if example.title:
            item = [self.item_vocab.index(w) if w in self.item_vocab else self.item_vocab.index(
                '[UNK]') for w in example.title]
        else:
...
class RnnDataset(Dataset):
    """
    This class will create dataset for training RNN based Model,and it's inherited 
    from torch map-style dataset. Each instance's attributes will use InputFeature 
    namedTuple class to save. So you can use _fields method to look attribute names.


    Args:
     examples(iterable object):Each instance in examples must be a namedTuple(InputFeature).
     vocab(list of str):The vocab instance for Loading text that releated with job & company.
     lda_vocab_path(str):The path for Loading gensim Dicitonary class.
     lda_model_path(str):The path for Loading gensim LDA class.
     args:Build from ArgumentParser class.
    ...
    Attributes:
     data(list of InputFeature):The object contain all InputFeature object.
     vocab(list of str):This is vocab for text fields that releated with job & company.Max Limit is 3.5w.
     item_vocab(list of str):This is job title field's vocab.
     lda_vocab(gensim Dicitonary instance):The lda_vocab that build from topic_model module.
     lda_model(gensim LDA instance):The lda_model that build from topic_model module.
    ...
    def __getitem__(self, key):
        """
        It's will return NamedTuple instance based on RNN Input format.
        Also you can use indexing method to query nth's data.
        """
        # fetch data
        example = self.data[key]
        ...
        # combine data
        features = allToken_ids + \
            [torch.tensor(item)]+[torch.tensor(meta_data)] + \
...
class process_data:
    """
    The universal data/text process pipeline.
    This pipeline is iterable object.

    Args:
     data(iterator object):The pandas dataframe which iterate row as namedtuple.
     args:Build from ArgumentParser class that should contain job/edu_thershold.


    Attributes:
     data(iterator object):Iterator with namedtuple row
     job_level(dict):Dict for job name mapping id.
     edu_level(dict):Dict for education name mapping id.
    """
    ...
    def __iter__(self):
        """
        This is a generator for uni preprocess data.
        """
    ...
    def extract_data(self, data):
        """
        This is universal process that access specific field value over namedtuple instance.

        Args:
         data(namedtuple object):This object is come from df.itertuples()

        Returns:
         NamedTuple object:This named tuple is saved for Inputfeature
        """
        # clean text
        cp_profile = text_clean(data.company_profile) if not pd.isna(
            data.company_profile) else ''
        desc = text_clean(data.description) if not pd.isna(
            data.description) else ''
        requires = text_clean(data.requirements) if not pd.isna(
            data.requirements) else ''
        benefits = text_clean(data.benefits) if not pd.isna(
            data.benefits) else ''
        title = text_clean(data.title)

        # create meta feature
        has_descLink = count_links(data.description) != 0 if not pd.isna(
            data.description) else 0
        ...
        meta_data = [has_descLink, require_edu,
                     require_job, lower_edu, lower_job]
        meta_data += [data.has_company_logo, data.telecommuting]
        # tokenized text
        cp_profile = doc_process(self.nlp_pipe(
            cp_profile.lower())) if cp_profile else []
        desc = doc_process(self.nlp_pipe(desc.lower())) if desc else []
        requires = doc_process(self.nlp_pipe(
            requires.lower())) if requires else []
        benefits = doc_process(self.nlp_pipe(
            benefits.lower())) if benefits else []
        title = [w.lower() for w in title.split() if (
            w not in string.punctuation) and w.isalpha()]
...
def create_mini_batch(batchs):
    """
    This func will collate list of examples
    to batch ditionary that corresponding attribute name.

    Args:
     batchs(list of example):The example from dataset sampled by torch Sampler class.

    Returns:
     dict: The key is input field name,and corresponding value is batch tensor.
    """
    ...
    if 'job_segs' in field_names:
        for f, v in list(batch_dict.items())[:2]:
            masks = torch.ones_like(v).masked_fill_(
                v == 0, 0)  # create mask tensors
            n = f.split('_')[0]
...
def balanced_process(dataset, class_weight, labels_name):
    """
    The func used for reduce positive example nums to 
    decrease model imbalance performance.So you can use
    weight to mutiply the numbers of negative exmaple as 
    times.And the sample method is random sample.

    Args:
     dataset(object):pands dataframe object.
     class weight(float):The times that mutilpy num of neg for sampling positive data.
     labels_name(str):The field name in dataset that you want to balance.

    Returns:
     df object:The dataframe object that after balanced process.
    ...
    logger.info(f'negative example nums after balanced:{len(neg_dataset)}')

    # combine  and shuffle data order
    dataset = neg_dataset.append(pos_dataset, ignore_index=True).sample(
...
def load_and_cacheEamxples(args, tokenizer, mode):
    """
    This func will preprocess dataset to  model
    input format's dataset.If you use command to read new datasets,
    it's will save universal processed dataset to local disk,else loading 
    dataset from disk according given dataset name.

    ##The dataset name will refer to task,training mode, balance weight.

    Args:
     args:Build from ArgumentParser class.
     tokenizer(object):The word vocab or process word's object.
     mode:the dataset type(train,val,set)

    Returns:
     object: The torch map-style dataset correspond to model's input format.
