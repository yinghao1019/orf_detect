...
from model_trainer import Model_pipe
from utils import *
...
def main(args):
    ...
    if args.used_model.startswith('bert'):
        ...
        tokenizer = MODEL_CLASSES[pretrained_name][1].from_pretrained(
            pretrained_name)
        ...
        logger.info(
            f'Vocab num that before add new spec token:{len(tokenizer)}')
        ...
        logger.info(
            f'Vocab num that after add new spec token:{len(tokenizer)}')
    ...
    if args.do_train:
        ...
        pipe = Model_pipe(train_data, val_data, model, args)
        pipe.train_model(save_model_dir)
    ...
    if args.do_eval:
        ...
        pipe = Model_pipe.load_model(
            pretrain_class, train_data, val_data, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--edu_threshold', type=int,
                        default=3, help='Lower edu threshold.')
    parser.add_argument('--job_threshold', type=int,
    ...
    parser.add_argument('--weight_decay', type=float,
                        default=0.0, help='weight decay for Adam.')
    ...
    parser.add_argument('--val_bs', type=int, default=32,
                        help='Eval model Batch size. Default is 32.')
    ...
    parser.add_argument('--warm_steps', type=int, default=100,
                        help='Linear Warm up steps.Default is 100.')
    ...
    parser.add_argument('--do_eval', action='store_true',
                        help='Whether to eval model or not.')
